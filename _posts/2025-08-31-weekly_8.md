...
<br>
YOLOv8의 구조
<img src="{{"/assets/images/yolov8.jpg" | relative_url }}" width="500" height="600" alt="yolov8">

1. Anchor-Free Detection: 
앵커 박스 개념을 사용하지 않아 Decoupled Head를 통해 모델 헤드가 단순화되고 학습이 안정되었습니다.
2. C2f 모듈: 
새로운 백본 및 Neck 구조를 도입하여 적은 파라미터로 효율적인 특징 추출을 가능하게 했습니다.
3. 통합 기능: 
...
#### YOLOv12
YOLOv12는 어텐션(Attention) 메커니즘을 적극적으로 활용하면서도 YOLO의 핵심인 속도를 유지한 모델입니다. 기존 어텐션 모델들은 높은 정확도에도 불구하고 연산량이 많아 속도가 느리다는 한계가 있었습니다. YOLOv12는 이러한 어텐션의 비효율성을 해결하기 위해 **영역 어텐션(Area Attention)**을 도입했습니다.
1. 어텐션 중심(Attention-Centric) 아키텍처: 
YOLOv12는 모델 전반에 영역 어텐션(A2) 모듈을 통합하여, 모델이 중요한 특징에 집중하게 함으로써 정확도를 높입니다.
2. 영역 어텐션(Area Attention): 
기존 어텐션은 모든 픽셀 간의 관계를 계산하여 막대한 연산량을 초래했지만, YOLOv12는 특정 영역 내에서만 어텐션을 수행하여 연산량을 크게 줄입니다.
3. 효율적인 성능: 
영역 어텐션을 통해 정확도를 높이면서도 CNN 기반 모델과 유사한 실시간 추론 속도를 유지할 수 있게 되었습니다.

- YOLOv12의 한계
어텐션 메커니즘은 아무리 경량화하더라도 기존 CNN 기반 모델보다 파라미터 수가 많아 모델이 무거워질 수 있습니다. 이로 인해 모바일이나 엣지 디바이스와 같은 저사양 환경에서는 여전히 속도 저하가 발생할 수 있습니다.
