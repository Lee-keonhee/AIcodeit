...
2. 각 Grid Cell에 대해 Bounding Box와 Classification 두 가지 Task를 동시에 진행한다.
   - 2-1. 각 Grid cell은 B개의 Bounding Box를 계산한다.
     ...
     - $$confidence = Pr(Object) * IoU(pred, true)$$  $$Pr(Object)$$은 Ground Truth의 Center point가 포함되면 1, 아니면 0이되는 확률값이고 IoU는 box간의 IoU를 의미한다.
...
3. NMS(Non-Max Suppression)를 거쳐 겹치는 Box들을 제거하고, 최종 결과만 보여준다.
   - NMS(Non-Max Suppression)
     ...
     <img src="{{"/assets/images/nms.png" | relative_url }}" width="200" height="150" alt="yolov1">
...
| 버전	| 주요 추가/변화 내용                                                                                                                      |
|-------|----------------------------------------------------------------------------------------------------------------------------------|
| YOLOv1 | 	• 최초의 One-stage 모델                                                                                                              |
| YOLOv2 | 	• 앵커 박스(Anchor Box) 도입                                                                                                          |
| YOLOv3 | 	• FPN(Feature Pyramid Network) + Skip connection 구조 도입                                                                          |
| YOLOv4 | 	• BoF, BoS 전략을 통한 최신 기법 결합                      |
| YOLOv5 | 	• PyTorch 기반 구현으로 뛰어난 사용 편의성 확보                                       |
| YOLOv6 | 	• 산업 응용에 최적화된 빠른 추론 속도                                                                      |
| YOLOv7 | 	• 역대 최고의 속도-정확도 트레이드오프 <br> • E-ELAN 아키텍처, Compound Model Scaling 도입                                                            |
| YOLOv8 | 	• Anchor-free 방식 채택 및 통합 프레임워크 제공                                                                 |
| YOLOv9 | 	• PGI 기법을 통한 정보 손실 문제 해결                                                                |
| YOLOv10 | 	• NMS-free 아키텍처로 End-to-End 추론 구현                                                    |
| YOLOv11 | 	• 듀얼 헤드 아키텍처로 작은 객체 탐지 성능 향상                                                    |
| YOLOv12 | 	• 어텐션 중심 아키텍처로 Transformer 기반 접근 시작 |
