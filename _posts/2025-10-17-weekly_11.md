...
BERT와 GPT라는 두 거대한 언어모델은 2018년도에 등장하였습니다. 두 모델 모두 Transformer 아키텍처를 기반으로 하지만, 접근 방식과 강점이 완전히 다릅니다. 

## 1.1 BERT: 양방향으로 문맥을 이해하는 모델
...
<strong>🎯 실전 팁:</strong> 텍스트를 이해하고 분류해야 한다면 BERT를, 텍스트를 생성하거나 대화를 나눠야 한다면 GPT를 선택하세요.<br> 최근에는 두 접근 방식을 결합한 BART, T5 같은 모델도 등장했습니다!
</div>
...
<div markdown="1" style="background-color: #3f2f2f; padding: 20px; border-left: 4px solid #ff1493; margin: 20px 0;">
...
<div markdown="1" style="background-color: #2f3f2f; padding: 20px; border-left: 4px solid #32cd32; margin: 20px 0;">
